webpackJsonp([1], {
    "4WSK": function(t, e) {},
    AqYs: function(t, e) {
        t.exports = "data:image/svg+xml;base64,"
    },
    NHnr: function(t, e, i) {
        "use strict";
        Object.defineProperty(e, "__esModule", {
            value: !0
        });
        var s = i("7+uW"),
            n = i("Xxa5"),
            a = i.n(n),
            u = i("Gu7T"),
            r = i.n(u),
            M = i("exGp"),
            c = i.n(M),
            L = i("//Fk"),
            o = i.n(L),
            l = i("bzuk"),
            w = i.n(l),
            C = i("hMDg"),
            D = i.n(C);
        for (var N in D.a) D.a[N] = D.a[N].toLowerCase();
        var E = {
                name: "App",
                components: {},
                data: function() {
                    return {
                        supportBluetooth: !1,
                        isConnected: !1,
                        serverId: "",
                        ssid: "",
                        password: "",
                        key: "",
                        wifiLock: !1,
                        characteristicsList: [],
                        infoList: [],
                        commandList: [],
                        commandOutput: "",
                        commandOutputShouldRefresh: !1,
                        loading: null,
                        charLength: -1,
                        customInfoCount: 0,
                        customCommandCount: 0,
                        isAndroid: navigator.userAgent.indexOf("Android") > -1 || navigator.userAgent.indexOf("Adr") > -1,
                        isIphone: navigator.userAgent.indexOf("iPhone") > -1 || navigator.userAgent.indexOf("iphone") > -1
                    }
                },
                mounted: function() {
                    this.supportBluetooth = window.navigator.bluetooth
                },
                methods: {
                    connectDevice: function() {
                        var t = this,
                            e = this;
                        navigator.bluetooth.requestDevice({
                            filters: [{
                                services: [D.a.SERVICE_ID]
                            }]
                        }).then(function(t) {
                            return e.loading = l.Loading.service({
                                fullscreen: !0
                            }), t.gatt.connect()
                        }).then(function(t) {
                            return e.serverId = t.device.id, t.getPrimaryService(D.a.SERVICE_ID)
                        }).then(function(e) {
                            return t.isIphone ? (console.log("ios webBLE"), t.webBleConnect(e)) : e.getCharacteristics()
                        }).then(function(t) {
                            e.characteristicsList = t, e.isConnected = !0, e.loading.close()
                        }).catch(console.log)
                    },
                    webBleConnect: function(t) {
                        var e, i = this,
                            s = this;
                        return new o.a((e = c()(a.a.mark(function e(n) {
                            var u, M, c, L, l, w;
                            return a.a.wrap(function(e) {
                                for (;;) switch (e.prev = e.next) {
                                    case 0:
                                        return e.next = 2, t.getCharacteristic(D.a.PREFIX + D.a.CUSTOM_INFO_COUNT).then(function(t) {
                                            return t.readValue()
                                        }).then(function(t) {
                                            return t.buffer
                                        }).then(i.ab2str).then(parseInt).then(function(t) {
                                            console.log("custom-info-count " + t), s.customInfoCount = t
                                        });
                                    case 2:
                                        return e.next = 4, t.getCharacteristic(D.a.PREFIX + D.a.CUSTOM_COMMAND_COUNT).then(function(t) {
                                            return t.readValue()
                                        }).then(function(t) {
                                            return t.buffer
                                        }).then(i.ab2str).then(parseInt).then(function(t) {
                                            console.log("custom-command-count " + t), s.customCommandCount = t
                                        });
                                    case 4:
                                        for (u = [], M = 0; M < s.customInfoCount; M++) c = (M + 1).toString(16), c = "0".repeat(4 - c.length) + c, u.push(D.a.PREFIX + D.a.CUSTOM_INFO + c), u.push(D.a.PREFIX + D.a.CUSTOM_INFO_LABEL + c);
                                        for (L = [], l = 0; l < s.customCommandCount; l++) w = (l + 1).toString(16), w = "0".repeat(4 - w.length) + w, L.push(D.a.PREFIX + D.a.CUSTOM_COMMAND_LABEL + w);
                                        n(o.a.all([t.getCharacteristic(D.a.SERVICE_NAME), t.getCharacteristic(D.a.DEVICE_MODEL), t.getCharacteristic(D.a.WIFI_NAME), t.getCharacteristic(D.a.IP_ADDRESS), t.getCharacteristic(D.a.NOTIFY_MESSAGE), t.getCharacteristic(D.a.INPUT_SEP), t.getCharacteristic(D.a.CUSTOM_COMMAND_INPUT), t.getCharacteristic(D.a.CUSTOM_COMMAND_NOTIFY)].concat(r()(u.map(function(e) {
                                            return t.getCharacteristic(e)
                                        })), r()(L.map(function(e) {
                                            return t.getCharacteristic(e)
                                        })))));
                                    case 9:
                                    case "end":
                                        return e.stop()
                                }
                            }, e, i)
                        })), function(t) {
                            return e.apply(this, arguments)
                        }))
                    },
                    ab2str: function(t) {
                        return String.fromCharCode.apply(null, new Uint8Array(t))
                    },
                    getCharacteristic: function(t) {
                        return this.characteristicsList.find(function(e) {
                            return e.uuid === t
                        })
                    },
                    subscribeCharacteristic: function(t) {
                        var e, i = this;
                        return new o.a((e = c()(a.a.mark(function e(s) {
                            return a.a.wrap(function(e) {
                                for (;;) switch (e.prev = e.next) {
                                    case 0:
                                        return i.getCharacteristic(t).addEventListener("characteristicvaluechanged", function(e) {
                                            if (e.target.uuid === D.a.NOTIFY_MESSAGE) {
                                                var s = i.ab2str(e.target.value.buffer),
                                                    n = i.$createElement;
                                                i.$notify({
                                                    title: "Error",
                                                    message: n("i", {
                                                        style: "color: teal"
                                                    }, s)
                                                })
                                            } else if (e.target.uuid === D.a.CUSTOM_COMMAND_NOTIFY) {
                                                var a = i.ab2str(e.target.value.buffer);
                                                i.commandOutputShouldRefresh && (i.commandOutputShouldRefresh = !1, i.commandOutput = "");
                                                var u = i.commandOutput + a;
                                                u.endsWith("&#&") && (u = u.replace("&#&", ""), i.commandOutputShouldRefresh = !0), i.commandOutput = u
                                            } else {
                                                var r = i.ab2str(e.target.value.buffer),
                                                    M = i.infoList.find(function(e) {
                                                        return e.uuid === t
                                                    }),
                                                    c = i.infoList.find(function(e) {
                                                        return e.uuid_label === t
                                                    });
                                                M && (M.value = r), c && (c.label = r)
                                            }
                                        }), e.next = 3, i.getCharacteristic(t).startNotifications();
                                    case 3:
                                        s();
                                    case 4:
                                    case "end":
                                        return e.stop()
                                }
                            }, e, i)
                        })), function(t) {
                            return e.apply(this, arguments)
                        }))
                    },
                    readInfoCharacteristic: function(t) {
                        var e = this;
                        return new o.a(function(i) {
                            e.getCharacteristic(t).readValue().then(function(t) {
                                return t.buffer
                            }).then(e.ab2str).then(function(s) {
                                var n = e.infoList.find(function(e) {
                                        return e.uuid === t
                                    }),
                                    a = e.infoList.find(function(e) {
                                        return e.uuid_label === t
                                    });
                                n && (n.value = s), a && (a.label = s), i()
                            })
                        })
                    },
                    readCommandLabel: function(t) {
                        var e = this;
                        return new o.a(function(i) {
                            e.getCharacteristic(t).readValue().then(function(t) {
                                return t.buffer
                            }).then(e.ab2str).then(function(s) {
                                e.commandList.find(function(e) {
                                    return e.uuid === t
                                }).label = s, i()
                            })
                        })
                    },
                    inputWifi: function() {
                        var t = this,
                            e = this.key.trim().replace(/\|/g, "*"),
                            i = this.ssid.trim().replace(/\|/g, "*"),
                            s = this.password.trim().replace(/\|/g, "*"),
                            n = void 0;
                        if ("" === i && (n = "SSID cannot be empty."), "" === s && (n = "Password cannot be empty."), n) {
                            var a = this.$createElement;
                            this.$notify({
                                title: "Error",
                                message: a("i", {
                                    style: "color: teal"
                                }, n)
                            })
                        } else {
                            this.wifiLock = !0, setTimeout(function() {
                                t.wifiLock = !1
                            }, 4e3);
                            var u = this.str2abs(e + "%&%" + i + "%&%" + s + "&#&");
                            this.sendSeparately(u, D.a.INPUT_SEP)
                        }
                    },
                    sendCommand: function(t) {
                        var e = this.str2abs(this.key + "%&%" + t.slice(-4) + "&#&");
                        this.sendSeparately(e, D.a.CUSTOM_COMMAND_INPUT)
                    },
                    sendSeparately: function(t, e) {
                        var i = this;
                        return c()(a.a.mark(function s() {
                            var n;
                            return a.a.wrap(function(s) {
                                for (;;) switch (s.prev = s.next) {
                                    case 0:
                                        s.t0 = a.a.keys(t);
                                    case 1:
                                        if ((s.t1 = s.t0()).done) {
                                            s.next = 9;
                                            break
                                        }
                                        return n = s.t1.value, s.next = 5, i.getCharacteristic(e).writeValue(t[n]);
                                    case 5:
                                        return s.next = 7, i.wait(.4);
                                    case 7:
                                        s.next = 1;
                                        break;
                                    case 9:
                                    case "end":
                                        return s.stop()
                                }
                            }, s, i)
                        }))()
                    },
                    wait: function(t) {
                        var e = this;
                        return c()(a.a.mark(function i() {
                            return a.a.wrap(function(e) {
                                for (;;) switch (e.prev = e.next) {
                                    case 0:
                                        return e.abrupt("return", new o.a(function(e) {
                                            setTimeout(function() {
                                                e(!0)
                                            }, 1e3 * t)
                                        }));
                                    case 1:
                                    case "end":
                                        return e.stop()
                                }
                            }, i, e)
                        }))()
                    },
                    str2abs: function(t) {
                        for (var e = "", i = 0; i < t.length; i++) "" === e ? e = t.charCodeAt(i).toString(16) : e += "," + t.charCodeAt(i).toString(16);
                        for (var s = e.split(","), n = []; s.length > 0;) {
                            var a = s.splice(0, 20).join(",");
                            n.push(new Uint8Array(a.match(/[\da-f]{2}/gi).map(function(t) {
                                return parseInt(t, 16)
                            })).buffer)
                        }
                        return n
                    }
                },
                watch: {
                    isConnected: function(t, e) {
                        var i = this;
                        return c()(a.a.mark(function s() {
                            var n, u;
                            return a.a.wrap(function(s) {
                                for (;;) switch (s.prev = s.next) {
                                    case 0:
                                        if (!0 !== t || !1 !== e) {
                                            s.next = 37;
                                            break
                                        }
                                        return console.log("Connected!"), i.infoList = [], i.infoList.push({
                                            preset: !0,
                                            uuid: "",
                                            label: "Device ID",
                                            value: i.serverId
                                        }), i.infoList.push({
                                            preset: !0,
                                            uuid: D.a.DEVICE_MODEL,
                                            label: "Model",
                                            value: " "
                                        }), i.infoList.push({
                                            preset: !0,
                                            uuid: D.a.WIFI_NAME,
                                            label: "Wifi",
                                            value: "..."
                                        }), i.infoList.push({
                                            preset: !0,
                                            uuid: D.a.IP_ADDRESS,
                                            label: "IP Address",
                                            value: "..."
                                        }), s.next = 9, i.subscribeCharacteristic(D.a.WIFI_NAME);
                                    case 9:
                                        return s.next = 11, i.subscribeCharacteristic(D.a.IP_ADDRESS);
                                    case 11:
                                        return s.next = 13, i.readInfoCharacteristic(D.a.DEVICE_MODEL);
                                    case 13:
                                        return s.next = 15, i.subscribeCharacteristic(D.a.NOTIFY_MESSAGE);
                                    case 15:
                                        return s.next = 17, i.subscribeCharacteristic(D.a.CUSTOM_COMMAND_NOTIFY);
                                    case 17:
                                        i.characteristicsList.filter(function(t) {
                                            return t.uuid.indexOf(D.a.CUSTOM_INFO_LABEL) >= 0
                                        }).map(function(t) {
                                            i.infoList.push({
                                                uuid: t.uuid.replace(D.a.CUSTOM_INFO_LABEL, D.a.CUSTOM_INFO),
                                                uuid_label: t.uuid,
                                                label: "-",
                                                value: ""
                                            })
                                        }), n = 0;
                                    case 19:
                                        if (!(n < i.infoList.length)) {
                                            s.next = 29;
                                            break
                                        }
                                        if (!i.infoList[n].preset) {
                                            s.next = 22;
                                            break
                                        }
                                        return s.abrupt("continue", 26);
                                    case 22:
                                        return s.next = 24, i.readInfoCharacteristic(i.infoList[n].uuid_label);
                                    case 24:
                                        return s.next = 26, i.subscribeCharacteristic(i.infoList[n].uuid);
                                    case 26:
                                        n++, s.next = 19;
                                        break;
                                    case 29:
                                        i.characteristicsList.filter(function(t) {
                                            return t.uuid.indexOf(D.a.CUSTOM_COMMAND_LABEL) >= 0
                                        }).map(function(t) {
                                            i.commandList.push({
                                                uuid: t.uuid,
                                                label: "..."
                                            })
                                        }), u = 0;
                                    case 31:
                                        if (!(u < i.commandList.length)) {
                                            s.next = 37;
                                            break
                                        }
                                        return s.next = 34, i.readCommandLabel(i.commandList[u].uuid);
                                    case 34:
                                        u++, s.next = 31;
                                        break;
                                    case 37:
                                    case "end":
                                        return s.stop()
                                }
                            }, s, i)
                        }))()
                    }
                }
            },
            j = {
                render: function() {
                    var t = this,
                        e = t.$createElement,
                        s = t._self._c || e;
                    return s("div", {
                        attrs: {
                            id: "app"
                        }
                    }, [s("img", {
                        staticClass: "logo",
                        attrs: {
                            src: i("AqYs")
                        }
                    }), t._v(" "), s("div", {
                        staticClass: "panel-container"
                    }, [t.supportBluetooth ? t._e() : s("div", {
                        staticClass: "connect-panel panel"
                    }, [s("div", {
                        staticClass: "panel-title"
                    }, [t._v("Cannot find web bluetooth.")]), t._v(" "), t._m(0)]), t._v(" "), !t.isConnected && t.supportBluetooth ? s("div", {
                        staticClass: "connect-panel panel"
                    }, [s("div", {
                        staticClass: "panel-title"
                    }, [t._v("Connect your pi ...")]), t._v(" "), s("el-row", [s("el-button", {
                        on: {
                            click: t.connectDevice
                        }
                    }, [t._v("Discover")])], 1)], 1) : t._e(), t._v(" "), t.isConnected ? s("div", {
                        staticClass: "info-panel panel"
                    }, [s("div", {
                        staticClass: "panel-title"
                    }, [t._v("Device Information")]), t._v(" "), t._l(t.infoList, function(e) {
                        return s("div", {
                            key: e.uuid,
                            staticClass: "info-group"
                        }, [s("div", {
                            staticClass: "label"
                        }, [t._v(t._s(e.label))]), t._v(" "), s("div", {
                            staticClass: "value"
                        }, [t._v(t._s(e.value))])])
                    })], 2) : t._e(), t._v(" "), t.isConnected ? s("div", {
                        staticClass: "wifi-panel panel"
                    }, [s("div", {
                        staticClass: "panel-title"
                    }, [t._v("Wifi Setting")]), t._v(" "), s("el-row", [s("el-input", {
                        attrs: {
                            placeholder: "Wifi name"
                        },
                        model: {
                            value: t.ssid,
                            callback: function(e) {
                                t.ssid = e
                            },
                            expression: "ssid"
                        }
                    }, [s("template", {
                        slot: "prepend"
                    }, [t._v("Wifi")])], 2)], 1), t._v(" "), s("el-row", [s("el-input", {
                        attrs: {
                            placeholder: "Wifi password",
                            type: "password"
                        },
                        model: {
                            value: t.password,
                            callback: function(e) {
                                t.password = e
                            },
                            expression: "password"
                        }
                    }, [s("template", {
                        slot: "prepend"
                    }, [t._v("Psw")])], 2)], 1), t._v(" "), s("el-row", [s("el-input", {
                        attrs: {
                            placeholder: "Key (Default: pisugar)"
                        },
                        model: {
                            value: t.key,
                            callback: function(e) {
                                t.key = e
                            },
                            expression: "key"
                        }
                    }, [s("template", {
                        slot: "prepend"
                    }, [t._v("Key")])], 2)], 1), t._v(" "), s("el-row", [s("el-button", {
                        attrs: {
                            disabled: t.wifiLock
                        },
                        on: {
                            click: t.inputWifi
                        }
                    }, [t._v("Submit")])], 1)], 1) : t._e(), t._v(" "), t.isConnected ? s("div", {
                        staticClass: "command-panel panel"
                    }, [s("div", {
                        staticClass: "panel-title"
                    }, [t._v("Custom Commands")]), t._v(" "), t.commandList.length > 0 ? [s("el-row", t._l(t.commandList, function(e) {
                        return s("div", {
                            key: e.uuid,
                            staticClass: "button-wrap"
                        }, [s("el-button", {
                            attrs: {
                                size: "small",
                                round: ""
                            },
                            on: {
                                click: function(i) {
                                    return t.sendCommand(e.uuid)
                                }
                            }
                        }, [t._v(t._s(e.label))])], 1)
                    }), 0), t._v(" "), s("el-row", [s("el-input", {
                        attrs: {
                            placeholder: "Key (Default: pisugar)"
                        },
                        model: {
                            value: t.key,
                            callback: function(e) {
                                t.key = e
                            },
                            expression: "key"
                        }
                    }, [s("template", {
                        slot: "prepend"
                    }, [t._v("Key")])], 2)], 1), t._v(" "), s("el-row", [s("el-input", {
                        attrs: {
                            type: "textarea",
                            placeholder: "Output",
                            rows: 10
                        },
                        model: {
                            value: t.commandOutput,
                            callback: function(e) {
                                t.commandOutput = e
                            },
                            expression: "commandOutput"
                        }
                    })], 1)] : s("p", [t._v("\n        No custom command is found.\n      ")])], 2) : t._e()]), t._v(" "), t._m(1)])
                },
                staticRenderFns: [function() {
                    var t = this.$createElement,
                        e = this._self._c || t;
                    return e("p", [this._v("Please make sure your device and browser support web bluetooth. Please visit "), e("a", {
                        attrs: {
                            href: "https://github.com/WebBluetoothCG/web-bluetooth/blob/master/implementation-status.md",
                            target: "_blank"
                        }
                    }, [this._v("Here")]), this._v(" to check web-bluetooth compatibility.")])
                }, function() {
                    var t = this.$createElement,
                        e = this._self._c || t;
                    return e("div", {
                        staticClass: "copyright"
                    }, [this._v("This Project is maintained by "), e("a", {
                        attrs: {
                            href: "https://github.com/PiSugar/sugar-wifi-conf",
                            target: "_blank"
                        }
                    }, [this._v("PiSugar Kitchen")]), this._v(". Open source under GPL-3.0 license.")])
                }]
            };
        var d = i("VU/8")(E, j, !1, function(t) {
            i("exdi")
        }, null, null).exports;
        i("4WSK");
        s.default.config.productionTip = !1, s.default.use(w.a), new s.default({
            el: "#app",
            components: {
                App: d
            },
            template: "<App/>"
        })
    },
    exdi: function(t, e) {},
    hMDg: function(t, e) {
        t.exports = {
            PREFIX:"00000000-0000-0000-0000-",
            SERVICE_ID:"FD2B4448-AA0F-4A15-A62F-EB0BE77A0000",
            SERVICE_NAME:"FD2B4448-AA0F-4A15-A62F-EB0BE77A0001",
            DEVICE_MODEL:"FD2B4448-AA0F-4A15-A62F-EB0BE77A0002",
            WIFI_NAME:"FD2B4448-AA0F-4A15-A62F-EB0BE77A0003",
            IP_ADDRESS:"FD2B4448-AA0F-4A15-A62F-EB0BE77A0004",
            INPUT:"FD2B4448-AA0F-4A15-A62F-EB0BE77A0005",
            NOTIFY_MESSAGE:"FD2B4448-AA0F-4A15-A62F-EB0BE77A0006",
            INPUT_SEP:"FD2B4448-AA0F-4A15-A62F-EB0BE77A0007",
            CUSTOM_COMMAND_INPUT:"FD2B4448-AA0F-4A15-A62F-EB0BE77A0008",
            CUSTOM_COMMAND_NOTIFY:"FD2B4448-AA0F-4A15-A62F-EB0BE77A0009",
            CUSTOM_INFO_LABEL:"FD2BCCCA",
            CUSTOM_INFO_COUNT:"FD2BCCAA0000",
            CUSTOM_INFO:"FD2BCCCB",
            CUSTOM_COMMAND_LABEL:"FD2BCCCC",
            CUSTOM_COMMAND_COUNT:"FD2BCCAC0000"
        }
    }
}, ["NHnr"]);
//# sourceMappingURL=app.0d790707b429e7d60379.js.map